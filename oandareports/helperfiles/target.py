...
class BaseDaskTarget(Target):
    ...
    @classmethod
    def _read(cls, path, storage_type, **kwargs):
        if storage_type == "parquet":
            return read_parquet(path, engine="fastparquet", **kwargs)
...
class ParquetTarget(BaseDaskTarget):
    ...
    :param path: directory or S3 link
    :param flag: Set to "_SUCCESS" for Hadoop or Spark

    :returns Read: A Dask dataframe
    :returns Write: Parquet file(s)"""
    ...
    def read(self, **kwargs):
        return BaseDaskTarget.read_dask(self, **kwargs)

    def write(self, collection, **kwargs):
        return BaseDaskTarget.write_dask(self, collection, **kwargs)
...
class CSVTarget(BaseDaskTarget):
    ...
    :returns Read: A Dask dataframe
    :returns Write: CSV file(s)"""

    def __init__(self, path, flag=""):
        self.flag = flag
        self.glob = "*.csv"
        self.path = path
        super(CSVTarget, self).__init__(self.path, self.glob)
